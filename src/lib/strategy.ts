...
export const getInitializeAaveV2StrategyData = (registry: string, bank: string, underlying: string, derivative: string) => {
...
export const getInitializeCompoundStrategyData = (registry: string, bank: string, underlying: string, derivative: string) => {
  ...
  const initializeData = strategyInterface.encodeFunctionData(
    ...
    [registry, bank, underlying, derivative, addresses.comp, addresses.compComptroller]
  );
  return initializeData;
};

export const getInitializeCurve3PoolStrategyData = (registry: string, bank: string, underlying: string, index: string) => {
  ...
  const initializeData = strategyInterface.encodeFunctionData(
    ...
    [registry, bank, underlying, addresses.crv3Token, addresses.crv, addresses.crv3Pool, addresses.crv3Gauge, addresses.crvMintr, index]
  );
  return initializeData;
};
